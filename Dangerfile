    ...
    failure.nodes.each do |f|
      f.match(/(test[a-z_\/]+.rb):([0-9]+)/) do |match|
        ...
        if !source_path.nil? && !line.nil?
          f = f.gsub(source_path + ':' + line, "<a href='https://github.com/#{github.pr_author}/spectral-workbench/tree/#{github.branch_for_head}/#{source_path}#L#{line}'>#{source_path}:#{line}</a>")
        end
